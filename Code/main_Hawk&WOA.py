...
def load_data():
    data_inputs = []
    with open('datasets/cancer_patient.csv','r') as csvfile:
    ...
    data_outputs = []
    with open('datasets/label_cancer.csv','r') as csvfile:
        rows = csv.reader(csvfile)
        for row in rows:
            row = numpy.array(row,dtype = float)
...
sigma = pow(((1.33*.707)/(.9064*1.5*pow(2,.25))),2/3)           #Levy flights
		...
		if abs(E) >= 1:
			if p < 0.3:
				...
				while(rand_hawk_index != i):
					rand_hawk_index = random.randrange(0,sol_per_pop)
				rand_hawk = pop_weights_mat[rand_hawk_index]
				pop_weights_mat[i] = Hawk.update_WOA(E, J, S, pop_weights_mat[i])
		else:
			if r >= .5 and abs(E) >= .5:
